...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action'], $_POST['reservation_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        if ($new_status === 'Approved') {
            ...
            $reservation_stmt = $conn->prepare("SELECT * FROM room_reservations WHERE id = ?");
            ...
            if ($reservation = $reservation_result->fetch_assoc()) {
                ...
                if (!$reservation['calendar_added']) {
                    $title = "Room Reserved: " . $reservation['room'];
                    $description = $reservation['purpose'];
                    $start_datetime = $reservation['reservation_date'] . ' ' . $reservation['start_time'];
                    $end_datetime = $reservation['reservation_date'] . ' ' . $reservation['end_time'];
                    $department = $reservation['department'];
                    $event_location = $reservation['room'];
                    $event_url = ''; // Optional

                    // Insert into activities table
                    $calendar_stmt = $conn->prepare("INSERT INTO activities (title, description, start_datetime, end_datetime, department, event_location, event_url) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    $calendar_stmt->bind_param("sssssss", $title, $description, $start_datetime, $end_datetime, $department, $event_location, $event_url);
                    $calendar_stmt->execute();
                    $calendar_stmt->close();

                    // Mark reservation as added to calendar
                    $mark_stmt = $conn->prepare("UPDATE room_reservations SET calendar_added = 1 WHERE id = ?");
                    $mark_stmt->bind_param("i", $reservation_id);
                    $mark_stmt->execute();
                    $mark_stmt->close();
                }
            }
...
<div class="container">
    ...
    <a href="../super_admin/dashboard.php" class="btn btn-back">Back to Dashboard</a>
</div>
